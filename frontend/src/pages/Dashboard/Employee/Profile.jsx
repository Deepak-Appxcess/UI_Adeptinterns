...
const handleOtpSubmit = async (e) => {
  ...
  try {
    ...
    // Only show organization form if not already completed
    if (!profile.has_completed_organization) {
...
<div className="bg-white rounded-xl shadow-lg p-6 mb-8">
  <div className="flex justify-between items-center mb-4">
    <h2 className="text-xl font-semibold text-gray-900">Profile Completion Status</h2>
    <div className="flex items-center">
      <span className="text-sm font-medium text-gray-500 mr-2">
        {profile.has_completed_organization ? 'Complete' : 'Incomplete'}
      </span>
      <div className={`w-3 h-3 rounded-full ${
        profile.has_completed_organization ? 'bg-green-500' : 'bg-yellow-500'
      }`}></div>
    </div>
  </div>
  ...
  <div className="space-y-4">
    <div className="flex justify-between items-center py-3 border-b border-gray-100">
      ...
      <span className={`text-sm font-medium ${
        profile.first_name && profile.last_name && profile.email && profile.phone_number
          ? 'text-green-600'
          : 'text-yellow-600'
      }`}>
        {profile.first_name && profile.last_name && profile.email && profile.phone_number
    ...
    <div className="flex justify-between items-center py-3 border-b border-gray-100">
      ...
      <span className={`text-sm font-medium ${
        profile.has_completed_organization ? 'text-green-600' : 'text-yellow-600'
      }`}>
        {profile.has_completed_organization ? 'Complete' : 'Incomplete'}
      </span>
    </div>
      ...
      <div className="flex items-start">
        ...
        <div>
          ...
          <p className="text-sm text-blue-700 mt-1">
            Your profile is currently hidden from job seekers. Complete your profile to increase visibility and unlock opportunities.
          </p>
          <button
            onClick={() => {
          ...
          >
            {!profile.is_phone_verified ? 'Verify Phone Number' : 'Complete Profile'}
          </button>
        </div>
      </div>
    </div>
  ...
  {showOrganizationForm && (
    ...
    >
      <motion.div
        ...
        className="bg-white rounded-xl w-full max-w-md sm:max-w-xl md:max-w-2xl p-4 sm:p-6 my-8 mx-auto max-h-[90vh] overflow-y-auto"
      >
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-sm text-yellow-700 mt-1">
                  Please verify your phone number to complete your profile setup.
                </p>
                <button
                  onClick={handlePhoneVerification}
                  className="mt-2 px-3 py-1 bg-yellow-600 text-white rounded text-sm hover:bg-yellow-700 transition-colors"
                  ...
                  <div className="space-y-4 sm:space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                      ...
                      <div>
                        ...
                        <input
                          type={organizationData.verification_method === 'gst' ? 'text' : 'text'}
                          name="verification_value"
                          value={organizationData.verification_value}
                          onChange={(e) => handleInputChange(e, 'organization')}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#18005F]/20 focus:border-[#18005F]"
                          placeholder={
                            organizationData.verification_method === 'gst' ? 'GST NO' :
